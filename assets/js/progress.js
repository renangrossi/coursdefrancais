/*!
 * Renan le Professeur — Gamification / Suivi de Progression (cours de français)
 * ------------------------------------------------------------------
...
(function () {
  ...
  var BADGES = [
    {
      id: "first_steps", icon: "🌱", name: "Premiers Pas",
      desc: "Termine ton premier exercice.",
      check: function (s) { return countExercisesDone(s) >= 1; },
    },
    {
      id: "perfectionist", icon: "🎯", name: "Perfectionniste",
      desc: "Obtiens 100 % à un exercice.",
      check: function (s) { return countPerfect(s) >= 1; },
    },
    {
      id: "streak_3", icon: "🔥", name: "Série de 3 Jours",
      desc: "Entraîne-toi 3 jours de suite.",
      check: function (s) { return s.streak.count >= 3; },
    },
    {
      id: "streak_7", icon: "🔥", name: "Série de 7 Jours",
      desc: "Entraîne-toi 7 jours de suite.",
      check: function (s) { return s.streak.count >= 7; },
    },
    {
      id: "placement_done", icon: "🧭", name: "Connais Ton Niveau",
      desc: "Termine le Test de Niveau.",
      check: function (s) { return !!s.pagesCompleted.placement; },
    },
    {
      id: "streak_14", icon: "🔥", name: "Série de 14 Jours",
      desc: "Entraîne-toi 14 jours de suite.",
      check: function (s) { return s.streak.count >= 14; },
    },
    {
      id: "streak_30", icon: "🔥", name: "Série de 30 Jours",
      desc: "Entraîne-toi 30 jours de suite.",
      check: function (s) { return s.streak.count >= 30; },
    },
    {
      id: "comeback", icon: "🔄", name: "Le Retour",
      desc: "Reviens après avoir rompu une série et termine un autre exercice.",
      check: function (s) { return !!(s.streak && s.streak.brokenOnce); },
    },
    {
      id: "first_test_yourself", icon: "📝", name: "Teste-toi, Testé",
      desc: "Termine pour la première fois une page Teste-toi.",
      check: function (s) {
    ...
    },
    {
      ...
      desc: "Obtiens 100 XP au total.",
      check: function (s) { return s.xp >= 100; },
    },
    {
      id: "xp_250", icon: "🌟", name: "250 XP",
      desc: "Obtiens 250 XP au total.",
      check: function (s) { return s.xp >= 250; },
    },
    {
      id: "xp_500", icon: "💫", name: "500 XP",
      desc: "Obtiens 500 XP au total.",
      check: function (s) { return s.xp >= 500; },
    },
    {
      id: "xp_1000", icon: "👑", name: "1000 XP",
      desc: "Obtiens 1000 XP au total.",
      check: function (s) { return s.xp >= 1000; },
    },
    {
      id: "no_hints_needed", icon: "💎", name: "Sans Indice",
      desc: "Obtiens 100 % à 5 exercices différents.",
      check: function (s) { return countPerfect(s) >= 5; },
    },
    {
      id: "sherlock", icon: "🕵️", name: "Sherlock",
      desc: "Cherche ton premier mot dans le dictionnaire.",
      check: function (s) { return (s.dictionaryUses || 0) >= 1; },
    },
    {
      id: "dictionary_power_user", icon: "📚", name: "Expert du Dictionnaire",
      desc: "Cherche 10 mots dans le dictionnaire.",
    ...
    },
    {
      ...
      id: "polyglot", icon: "🌍", name: "Chemin Polyglotte",
      desc: "Termine au moins un exercice dans 3 niveaux différents.",
      check: function (s) { return countLevelsWithActivity(s) >= 3; },
    },
    {
      id: "night_owl", icon: "🦉", name: "Oiseau de Nuit",
      desc: "Termine un exercice entre minuit et 5 h du matin (selon l'horloge de ton appareil).",
      check: function (s) { return !!(s.timeFlags && s.timeFlags.nightOwl); },
    },
    {
      id: "early_bird", icon: "🐦", name: "Lève-tôt",
      desc: "Termine un exercice entre 5 h et 7 h du matin (selon l'horloge de ton appareil).",
    ...
    return {
      ...
      icon: "🏅",
      name: level + " Explorateur",
      desc: "Termine " + threshold + " exercices en " + level + " (" + LEVEL_NAMES[level] + ").",
      level: level,
      threshold: threshold,
      check: function (s) {
  ...
  function evaluateBadges(state) {
    BADGES.forEach(function (b) {
      ...
      pendingToasts.push({
        kind: "badge",
        text: b.name + " débloqué" + (xpAwarded > 0 ? " · +" + xpAwarded + " XP" : ""),
        icon: b.icon,
      });
    });
  }
  ...
  function maybeCompleteTopic(state, level, inventory) {
    ...
    pendingToasts.push({
      kind: "badge",
      text: "Sujet terminé : " + name + (xpAwarded > 0 ? " · +" + xpAwarded + " XP" : ""),
      icon: "📘",
    });
  }
  ...
  function recordTestProgressInternal(state, opts) {
    ...
    if (type === "placement") {
      awardXp(state, XP.placement, null, "Test de Niveau terminé");
    } else {
      awardXp(state, XP.testYourself, level, (level || "") + " Teste-toi terminé");
    }
  ...
  function recordExerciseResult(opts) {
    ...
    if (!existing) {
      ...
      awardXp(state, XP.exercise, level, "Exercice terminé");
      if (perfect) awardXp(state, XP.perfectBonus, level, "Score parfait");
  ...
  function touchStreakAndDailyBonus(state) {
    ...
    if (res.isNewDay) {
      pendingToasts.push({ kind: "streak", text: "Série de " + state.streak.count + " jours !" });
      if (XP.dailyBonus > 0 && wasCount > 0) awardXp(state, XP.dailyBonus, null, "Bonus quotidien");
    }
  }
  ...
  function buildWidget() {
    ...
    var toggle = el("button", {
      ...
      "aria-expanded": "false",
      "aria-label": "Ta progression : XP, série et badges",
    });
    ...
    if (dictItem) {
      ...
      menuToggle.appendChild(el("span", { class: "progress-menu-toggle__label", text: "Progression" }));
    ...
    var panel = el("div", { class: "progress-panel", role: "dialog", "aria-label": "Ta progression" });
    panel.hidden = true;
  ...
  function topicSummaryHtml(state) {
    ...
    if (!n) return "";
    return '<p class="progress-panel__hint">📘 ' + n + (n === 1 ? " sujet terminé." : " sujets terminés.") + "</p>";
  }

  // Full list, used on progress.html's dedicated "Topics" section.
  function topicListHtml(state) {
    var names = topicNames(state);
    if (!names.length) return '<p class="progress-panel__hint">Tu n\'as encore terminé aucun sujet — termine tous les exercices d\'une leçon pour en obtenir un.</p>';
  ...
  function renderPanel(state) {
    ...
    var levelRows = LEVELS.map(function (l) {
      ...
      return (
        ...
        '<div class="progress-label"><span>' + l + "</span><span>" + ls.exercisesDone + " faits · " + ls.xp + " XP</span></div>" +
    ...
    els.panel.innerHTML =
      ...
      '<div><strong>' + state.xp + ' XP</strong><span class="progress-panel__streak-label">🔥 Série de ' + state.streak.count + ' jours</span></div>' +
      '<button type="button" class="progress-panel__close" aria-label="Fermer">' +
      ...
      "</div>" +
      '<p class="progress-panel__hint">La progression est enregistrée uniquement dans ce navigateur — sûre au rechargement, sans besoin de compte.</p>' +
      '<div class="progress-panel__levels">' + levelRows + "</div>" +
      topicSummaryHtml(state) +
      '<p class="progress-panel__badges-label">Badges — ' + earnedCount + " sur " + BADGES.length + " obtenus</p>" +
      '<ul class="badge-grid">' + badgeGridHtml(state) + "</ul>" +
      '<a class="btn btn--ghost btn--small progress-panel__link" href="' + progressPageHref() + '">Voir la progression complète</a>';
  ...
  function renderToggle(state) {
    ...
    if (els.menuToggle) {
      ...
      els.menuToggle.setAttribute(
        "aria-label",
        "Ta progression : " + state.xp + " XP, série de " + state.streak.count + " jours"
      );
    }
  ...
  function renderProgressPage() {
    ...
    function render() {
      ...
      if (summaryEl) {
        summaryEl.innerHTML =
          ...
          '<div class="progress-stat"><strong>' + state.streak.count + '</strong><span>Jours de série</span></div>' +
          '<div class="progress-stat"><strong>' + state.badges.length + " / " + BADGES.length + '</strong><span>Badges obtenus</span></div>' +
          '<div class="progress-stat"><strong>' + countExercisesDone(state) + '</strong><span>Exercices terminés</span></div>';
      }
      if (levelsEl) {
        levelsEl.innerHTML = LEVELS.map(function (l) {
          ...
          return (
            '<div class="progress-panel__level-row">' +
            '<div class="progress-label"><span>' + l + " — " + LEVEL_NAMES[l] + "</span><span>" + ls.exercisesDone + " faits · " + ls.xp + " XP</span></div>" +
            '<div class="progress-track"><div class="progress-track__fill" style="width:' + pct + '%"></div></div>' +
            "</div>"
          );
        }).join("");
    ...
    if (resetBtn) {
      resetBtn.addEventListener("click", function () {
        if (window.confirm("Réinitialiser tout le XP, la série et les badges sur cet appareil ? Cette action est irréversible.")) {
          resetProgress();
          render();
        }
      });
