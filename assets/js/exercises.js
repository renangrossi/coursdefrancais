/*!
 * Renan le Professeur — Moteur d'Exercices Interactifs (cours de français)
 * ------------------------------------------------------------------
...
(function () {
  ...
  function setFeedback(fbRef, correct, correctAnswerText, iconWrap) {
    ...
    fbRef.strongEl.textContent = correct ? "Correct !" : "Ce n'est pas correct.";
    if (!correct && correctAnswerText) {
      var existing = fbRef.body.querySelector(".correct-answer");
      if (!existing) {
        var correctAnswerEl = el("div", { class: "correct-answer", html: "<em>Réponse correcte :</em> " + correctAnswerText });
  ...
  function renderChoice(item, index) {
    ...
    fieldset.appendChild(el("legend", { class: "visually-hidden", text: "Choisis une réponse" }));
      ...
      grade: function () {
        ...
        optionEls.forEach(function (o) {
          ...
          if (o.origIndex === item.answerIndex) {
            ...
            o.label.appendChild(el("span", { class: "option__tag", "aria-hidden": "true", text: "\u2713 correcte" }));
          } else if (o.origIndex === chosen) {
            o.label.classList.add("is-incorrect");
            o.label.appendChild(el("span", { class: "option__tag", "aria-hidden": "true", text: "\u2717 ta r\u00e9ponse" }));
          }
        });
  ...
  function renderTrueFalse(item, index) {
    ...
    var fieldset = el("fieldset");
    fieldset.appendChild(el("legend", { class: "visually-hidden", text: "Vrai ou faux" }));
    ...
    var trueLabel = el("label", { class: "option", for: trueId }, [trueInput, el("span", { class: "option__label", text: "Vrai" })]);
    var falseLabel = el("label", { class: "option", for: falseId }, [falseInput, el("span", { class: "option__label", text: "Faux" })]);
    ...
    return {
      ...
      grade: function () {
        ...
        correctLabel.appendChild(el("span", { class: "option__tag", "aria-hidden": "true", text: "\u2713 correcte" }));
        if (chosen !== null && chosen !== item.answer) {
          var wrongLabel = chosen ? trueLabel : falseLabel;
          wrongLabel.classList.add("is-incorrect");
          wrongLabel.appendChild(el("span", { class: "option__tag", "aria-hidden": "true", text: "\u2717 ta r\u00e9ponse" }));
        }
        wrap.classList.add("is-locked");
        setFeedback(fb, correct, null, iconWrap);
  ...
  function renderCorrection(item, index) {
    ...
    label.appendChild(document.createTextNode("Écris la phrase correcte :"));
  ...
  function renderTyping(item, index) {
    var wrap = itemShell(index, item.prompt);
    var input = el("input", { type: "text", class: "answer-input", autocomplete: "off", spellcheck: "false", "aria-label": item.prompt || "Ta réponse" });
    wrap.appendChild(input);
    ...
    return {
      ...
      grade: function () {
        ...
        if (selfCheck) {
          ...
          fb.strongEl.textContent = item.modelAnswer ? "Réponse modèle :" : "Enregistré pour ta propre révision.";
  ...
  function renderOrdering(item, index) {
    var wrap = itemShell(index, item.prompt || "Remets les mots dans le bon ordre.");
    var buildArea = el("div", { class: "order-build", role: "list", "aria-label": "Ta phrase" });
    var pool = el("div", { class: "order-pool", role: "list", "aria-label": "Mots disponibles" });
  ...
  function buildBlock(container, data) {
    ...
    var submitBtn = el("button", { type: "button", class: "btn btn--accent", text: "Vérifier" });
    var retryBtn = el("button", { type: "button", class: "btn btn--ghost", text: "Recommencer seulement les incorrectes" });
    var retryAllBtn = el("button", { type: "button", class: "btn btn--ghost", text: "Recommencer tout" });
    var printBtn = el("button", { type: "button", class: "btn btn--ghost print-hidden", text: "Enregistrer mes réponses" });
    ...
    function showScore(results) {
      ...
      var headingText = pct === 100 ? "Excellent, score parfait !" : pct >= 80 ? "Très bien !" : pct >= 50 ? "Bon progrès." : "Continue à t'entraîner.";
      var subText = correctCount + " sur " + total + " correctes (" + pct + " %).";
  ...
  function buildWritingBlock(container, data) {
    var head = el("div", { class: "exercise-block__head" });
    head.appendChild(el("span", { class: "exercise-block__type", text: "écriture" }));
    head.appendChild(el("h3", { class: "exercise-block__title", text: data.title || "Écriture" }));
    if (data.instructions) head.appendChild(el("p", { class: "exercise-block__instructions", text: data.instructions }));
    container.appendChild(head);
    ...
    (data.items || []).forEach(function (item, i) {
      ...
      var textarea = el("textarea", {
        ...
        "aria-label": item.prompt || "Ta réponse",
      });
    ...
    var actions = el("div", { class: "exercise-actions" });
    var saveBtn = el("button", { type: "button", class: "btn btn--ghost print-hidden", text: "Enregistrer mes réponses" });
    ...
    saveBtn.addEventListener("click", function () {
      performSaveAnswers(container, data, function () {
        ...
        printWrap.appendChild(el("h3", { class: "exercise-block__title", text: data.title || "Écriture" }));
        textareas.forEach(function (ta, i) {
          ...
          answerP.textContent = ta.value.trim() || "(Tu n'as pas encore écrit de réponse.)";
  ...
  function buildResultItemNode(entry) {
    ...
    wrap.appendChild(el("p", {
      ...
      html: "<strong>Ta réponse :</strong> " + (entry.userAnswer ? escapeHtml(entry.userAnswer) : "<em>(Tu n'as pas donné de réponse)</em>"),
    }));
    if (entry.correctAnswer) {
      wrap.appendChild(el("p", {
        class: "saved-summary-item__correct",
        html: "<strong>" + (entry.result === "self-check" ? "Réponse modèle :" : "Réponse correcte :") + "</strong> " + escapeHtml(entry.correctAnswer),
      }));
    }
  ...
  function performTopicSave(topicSection) {
    ...
    var fakeData = { type: null, title: "Tous les Exercices du Sujet — Réponses Enregistrées" };
  ...
  function performTestSave() {
    ...
    var fakeData = { type: null, title: "Teste-toi — Tous les Sujets — Réponses Enregistrées" };
  ...
  function buildSaveAllAnswersButton(ariaLabel, onClick) {
    var label = "Enregistrer Toutes les Réponses";
    var btn = el("button", {
      type: "button",
  ...
  // `root` that actually has a Submit ("Vérifier") button -- i.e.
  // the last *graded* block, searching backwards through document
  // order. A "writing" block's actions row only ever has a
  // "Enregistrer mes réponses" button (no grading, so no Submit), so
  ...
  function findLastSubmitActionsRow(root) {
    ...
    for (var i = blocks.length - 1; i >= 0; i--) {
      ...
      if (firstBtn && firstBtn.textContent === "Vérifier") return actions;
    }
    return null;
  }

  // Inserts `btn` as the row's second control, immediately after
  // Submit -- [Vérifier] [Enregistrer Toutes les Réponses] -- rather than at
  ...
  function addPracticeSaveAllButton() {
    ...
    var label = heading ? heading.textContent.trim() : "Pratique";

    var btn = buildSaveAllAnswersButton(
      "Enregistrer toutes les réponses : " + label + "",
  ...
  // "Enregistrer Toutes les Réponses" button beside its own last Submit button,
  ...
  function addTestYourselfTopicSaveButtons() {
    document.querySelectorAll(".ty-topic[id]").forEach(function (topicSection) {
      ...
      if (!actionsRow) return;
      var heading = topicSection.querySelector(".section__head h2, h2, h3");
      var topicTitle = heading ? heading.textContent.trim() : "ce sujet";
      var btn = buildSaveAllAnswersButton(
        "Enregistrer toutes les réponses du sujet : " + topicTitle,
        function () { performTopicSave(topicSection); }
      );
      insertBesideSubmit(actionsRow, btn);
    });
  ...
  function submitUnsubmittedBlocksIn(roots) {
    roots.forEach(function (root) {
      root.querySelectorAll(".exercise-block").forEach(function (block) {
        ...
        if (submitBtn && submitBtn.textContent === "Vérifier" && submitBtn.style.display !== "none") submitBtn.click();
      });
    });
  }
  ...
  function performGenericSave(roots, label) {
    ...
    var fakeData = { type: null, title: label + " — Réponses Enregistrées" };
    performSaveAnswers(document.body, fakeData, function () {
      var wrap = el("div", { class: "exercise-block saved-summary-root" });
  ...
  function init() {
    document.querySelectorAll(".exercise-block").forEach(function (container) {
      ...
      try {
        ...
        container.innerHTML = "<p>Impossible de charger cet exercice.</p>";
  ...
  function maybeAddTestSaveButton() {
    ...
    var btn = el("button", {
      ...
      text: "Enregistrer Toutes les Réponses de Teste-toi",
      "aria-label": "Enregistrer toutes les réponses de chaque sujet de Teste-toi" + (levelCode ? " de " + levelCode : ""),
    });
    btn.addEventListener("click", function () {
      performTestSave();
    });
