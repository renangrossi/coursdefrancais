/*!
 * Renan le Professeur — AI French Teacher chat widget
 * -----------------------------------------------------------------
 * Ported from the sibling Spanish-course project's assets/js/
...
(function () {
  ...
  /* ---------------------------------------------------------------
   * "Ç" standard intro — draws attention to the AI Teacher button
   ...
   * from it bearing a celeste-ringed gold "Ç" — the cedilla, the
   * diacritic most emblematic of French itself (it's the letter that
   * makes "français" spell itself correctly). French is spoken across
   * many countries, so rather than pick one nation's flag (the way the
  ...
  var CEDILLA_STANDARD_SVG =
    '<svg class="ai-teacher-intro__banner" viewBox="0 0 60 64" role="img" aria-label="Étendard avec la lettre Ç">' +
    // Pole + ball finial
    ...
    // Celeste ring + gold "Ç" lettering, centered on the banner
    '<circle cx="18" cy="32.5" r="8.6" fill="none" stroke="var(--forest-600)" stroke-width="0.9"/>' +
    '<text x="18" y="36.5" text-anchor="middle" font-family="Fraunces, Georgia, serif" font-size="10" font-weight="700" fill="var(--gold-300)">Ç</text>' +
    "</svg>";
  ...
  function playFlagIntro() {
    ...
    intro.innerHTML = CEDILLA_STANDARD_SVG;
  ...
  // "Adjectifs et Accord — Grammaire Française A1 — Renan le
  // Professeur" -> "Adjectifs et Accord"). The Worker re-validates
  ...
  var SITE_BASE_URL = "https://renangrossi.github.io/coursdefrancais/";
  ...
  //      label to reuse).
  //   2. A bare *relative* site path (e.g. "levels/pre-a1/alphabet-et-sons-du-francais.html"
  ...
  form.addEventListener("submit", function (e) {
    ...
    fetch(ENDPOINT, {
      ...
      .then(function (data) {
        typing.remove();
        var reply = data && data.reply ? data.reply : "Désolé, je n'ai pas reçu de réponse. Réessaie, s'il te plaît.";
      ...
      .catch(function (err) {
        ...
        if (err && err.message === "RATE_LIMIT") {
          addMessage("bot", "Tu as atteint la limite de questions d'aujourd'hui pour le Professeur IA. Reviens demain, ou continue à t'entraîner en attendant avec les exercices du cours !");
        } else {
          addMessage("bot", "Désolé, je n'ai pas pu me connecter au Professeur IA pour le moment. Vérifie ta connexion et réessaie dans un instant.");
        }
      })
      .finally(function () {
        setBusy(false);
