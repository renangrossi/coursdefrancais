/*!
 * Renan le Professeur — Recherche du Site (cours de français)
 * ------------------------------------------------------------------
...
(function () {
  ...
  var TYPE_LABEL = {
    ...
    grammar: "Sujet de Grammaire",
    booklet: "Livret de Grammaire",
    exercise: "Exercice / Texte",
    mock: "Examen Blanc",
    extra: "Extra",
  };
    ...
    indexPromise = fetch(indexSrc)
      ...
      .catch(function (err) {
        ...
        resultsBox.innerHTML =
          '<p class="search-modal__hint">Impossible de charger l\u2019index de recherche. Si tu vois cette page comme un fichier local ' +
          "(une URL qui commence par <code>file://</code>), c\u2019est normal \u2014 les navigateurs bloquent ce type de requ\u00eate pour les fichiers locaux. " +
          "Lance un serveur local (par exemple, <code>python3 -m http.server</code> dans le dossier du site) ou visite le site publi\u00e9.</p>";
        return [];
      });
    return indexPromise;
  }
  ...
  function runSearch(query) {
    ...
    if (q.length < 2) {
      resultsBox.innerHTML = '<p class="search-modal__hint">Tape au moins 2 caractères pour chercher dans tous les niveaux, leçons, sujets de grammaire, exercices et examens blancs.</p>';
      activeIndex = -1;
      return;
    }
    ...
    if (!scored.length) {
      resultsBox.innerHTML = '<p class="search-modal__hint">Aucun résultat pour &ldquo;' + escapeHtml(query) + '&rdquo;. Essaie un autre mot ou explore par niveau dans la navigation.</p>';
      activeIndex = -1;
      return;
    }
