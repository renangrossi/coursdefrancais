/*!
 * Renan le Professeur — Widget Flottant de Dictionnaire
 * Un petit outil de recherche flottant (avec le même style que le
 * bouton de retour en haut), disponible sur chaque page, pour que
 * l'étudiant puisse consulter un mot en français sans perdre sa
 * place. Interroge directement l'API REST du Wiktionnaire en français
 * (gratuite, sans clé, avec CORS activé) et ne garde que les entrées
 * dont la langue est "Français" — le Wiktionnaire en français
 * documente les mots avec des définitions en français, exactement
 * l'immersion monolingue qu'utilise tout ce site (pas de traduction
 * vers l'anglais). La prononciation n'a pas de clips audio dans cette
 * source, donc elle recourt toujours directement à speechSynthesis du
 * navigateur, lisant le mot à voix haute avec une voix en français.
 * Les liens vers des dictionnaires externes sous le résultat
 * s'affichent toujours aussi, dans le même ordre fixe que la page
 * principale Dictionnaire, comme une deuxième façon de consulter le
 * mot.
 */
(function () {
  "use strict";
  ...
  function renderOutboundLinks(word) {
    var encoded = encodeURIComponent(word || "bonjour");
    // Same fixed order as the Dictionary page's core cards.
    var sites = [
      ["Larousse", "https://www.larousse.fr/dictionnaires/francais/" + encoded],
      ["Wiktionnaire", "https://fr.wiktionary.org/wiki/" + encoded],
      ["Académie française", "https://www.dictionnaire-academie.fr/article/" + encoded],
      ["Synonymes", "https://www.cnrtl.fr/synonymie/" + encoded],
    ];
  ...
  /* ---------------------------------------------------------------
   * Wiktionnaire en français, endpoint REST, filtré sur la section de
   * langue "Français" — voir le commentaire d'en-tête du fichier.
   * --------------------------------------------------------------- */
  ...
  function fetchWiktionaryDefinition(word) {
    return fetch("https://fr.wiktionary.org/api/rest_v1/page/definition/" + encodeURIComponent(word))
      .then(function (res) {
        if (!res.ok) throw new Error("not found: " + word);
        return res.json();
      })
      .then(function (data) {
        // Le Wiktionnaire en français regroupe les entrées par la
        // langue *à laquelle appartient le mot* — seules les sections
        // réellement françaises sont conservées.
        var groups = ((data && data.fr) || []).filter(function (g) {
          return g.language === "Français" || g.language === "français";
        });
        if (!groups.length) throw new Error("aucune entrée en français : " + word);
  ...
  function lookup(word) {
    word = word.trim();
    if (!word) {
      resultBox.innerHTML = '<p class="dict-widget__hint">Écris un mot et appuie sur Entrée, ou attends un instant après avoir écrit.</p>';
      linksBox.innerHTML = "";
      return;
    }
    ...
    resultBox.innerHTML = '<p class="dict-widget__hint">Recherche de &ldquo;' + escapeHtml(word) + '&rdquo;&hellip;</p>';
    lastQuery = word;
    ...
    tryCandidates(candidateWords(word))
      ...
      .catch(function () {
        ...
        resultBox.innerHTML =
          '<p class="dict-widget__hint">Prêt à chercher &ldquo;' + escapeHtml(word) + '&rdquo;. Choisis un dictionnaire ci-dessous :</p>';
      });
  }

  var currentUtterance = null;

  // Prononciation : la voix de synthèse vocale du navigateur lui-même,
  // lue avec une voix en français quand l'appareil en a une
  // disponible. Cette source n'a pas de clips audio propres, donc la
  // synthèse vocale est le seul chemin (pas un repli après un clip en
  // échec).
  ...
  function playPronunciation(word, btn) {
    ...
    btn.setAttribute("title", "Ce navigateur ne peut pas lire à voix haute en français — essaie un lien de dictionnaire ci-dessous.");
  }

  function renderDefinition(data) {
    if (!Array.isArray(data) || !data.length) {
      resultBox.innerHTML = '<p class="dict-widget__hint">Aucune définition trouvée. Essaie un dictionnaire ci-dessous.</p>';
      return;
    }
    var entry = data[0];
    ...
    audioBtn.setAttribute("aria-label", "Écouter la prononciation en français");
