# Professeur IA — backend deployment
...
> limit counters. It's been fixed here (`ai-teacher-fr`, placeholder
> KV id) — if you deploy from a different checkout, double check
> `wrangler.toml` says `ai-teacher-fr`, not `ai-teacher`, before running
> `wrangler deploy`.
...
```
This is just the scheme+host of GitHub Pages — the same value works for every repo/project page under that account, so it's already correct for `coursdefrancais`. Update it only if the site moves to a custom domain.
...
So Professeur IA can recommend a *real* lesson link instead of guessing
...
- `type` is `"lesson"` (a level lesson page — grammar explanation AND its own exercises together), `"test"` (a level's "Teste-toi" cumulative review page), or `"page"` (site-wide utility pages: Exercices, Dictionnaire, Test de Niveau, Examens Blancs, Extras, Verbes Irréguliers, Révision du Jour, Ma Progression).
